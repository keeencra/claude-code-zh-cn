...
[![macOS native](https://img.shields.io/badge/macos%20native-2.1.113--2.1.274-green)](./docs/support-matrix.md)
[![Linux native](https://img.shields.io/badge/linux%20native-2.1.220--2.1.274-green)](./docs/support-matrix.md)
[![Windows native](https://img.shields.io/badge/windows%20native-2.1.113--2.1.274-green)](./docs/support-matrix.md)
...
| `curl -fsSL https://claude.ai/install.sh \| bash -s 2.1.274` | Linux x64 glibc 已验证版本（需要 `node-lief >=1.3.0`）；不含 arm64、musl 或未验证版本 |
| `powershell -File install.ps1` | Windows：旧 npm cli.js 最完整；native .exe `2.1.113 - 2.1.274` 内已验证版本需 `node-lief`；Claude 更新后关闭所有窗口并重跑 |
...
| macOS · native binary（arm64） | `2.1.113 - 2.1.274` 内的已验证版本 | 需要 `node-lief`；个别版本未收录，见支持矩阵 |
| Linux · native binary（x64 glibc） | `2.1.220 - 2.1.274` | 需要 `node-lief >=1.3.0`；未收录版本先本机验证，不含 arm64、musl |
| Windows · npm（PowerShell） | `2.1.92 - 2.1.112` | 用 install.ps1，需 PowerShell 5.1+ |
| Windows · native .exe（x64） | `2.1.113 - 2.1.274` 内的已验证版本 | 需要 `node-lief`；个别版本未收录，见支持矩阵 |
