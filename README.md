...
[![macOS native](https://img.shields.io/badge/macos%20native-2.1.113--2.1.278-green)](./docs/support-matrix.md)
[![Linux native](https://img.shields.io/badge/linux%20native-2.1.220--2.1.278-green)](./docs/support-matrix.md)
...
| `curl -fsSL https://claude.ai/install.sh \| bash -s 2.1.278` | Linux x64 glibc 已验证版本（需要 `node-lief >=1.3.0`）；不含 arm64、musl 或未验证版本 |
...
| macOS · native binary（arm64） | `2.1.113 - 2.1.278` 内的已验证版本 | 需要 `node-lief`；个别版本未收录，见支持矩阵 |
| Linux · native binary（x64 glibc） | `2.1.220 - 2.1.278` | 需要 `node-lief >=1.3.0`；未收录版本先本机验证，不含 arm64、musl |
